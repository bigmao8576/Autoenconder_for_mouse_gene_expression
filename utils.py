...
def file2dict(path,channel_num=2,norm=True):
    ...
    for item in data_list:
        temp_data = np.float32(np.array(item[1:]))
        if norm:
...
def draw_pic_cost(plotdata,save_folder):
    ...
    plt.plot(np.log10(np.array(plotdata["train_cost"])),'b') 
    
    if "test_cost" in plotdata:
        plt.plot(np.log10(np.array(plotdata["test_cost"])),'r')
        plt.legend(['Training _set','Testing_set'])
        
    else:
        plt.legend(['Training _set'])
    
    plt.xlabel('Epoch * %d'%plotdata['config']['int_ep'])
...
def draw_pic_sample(x_train,train_result_output,save_folder):
    ...
    plt.legend(['original seq','recovered_seq'])
    file_name = os.path.join(save_folder,'sample1.png')
    plt.savefig(file_name)  
    plt.cla() 
    plt.clf()
    plt.close('all')         
    ...
    plt.legend(['original seq','recovered_seq'])
    file_name = os.path.join(save_folder,'sample2.png')
    plt.savefig(file_name)  
